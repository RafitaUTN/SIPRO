(function () {
  ...
  function closeModal() { if (!activeModal) return; const modal = activeModal; activeModal = null; modal.remove(); document.body.classList.remove('modal-open'); returnFocus?.focus(); }
  function openModal({ title, subtitle = '', icon = 'square-pen', content, submitLabel, onSubmit, variant = 'primary' }) {
    closeModal(); returnFocus = document.activeElement;
    const overlay = document.createElement('div'); overlay.className = 'modal-overlay'; overlay.innerHTML = `<section class="app-modal" role="dialog" aria-modal="true" aria-labelledby="modalTitle"><header class="modal-head"><div class="modal-head-left"><div class="modal-icon"><i data-lucide="${icon}" aria-hidden="true"></i></div><div><h2 id="modalTitle">${title}</h2>${subtitle ? `<div class="modal-subtitle">${subtitle}</div>` : ''}</div></div><button type="button" class="modal-close" data-modal-close aria-label="Cerrar"><i data-lucide="x" aria-hidden="true"></i></button></header><div class="modal-body"></div><footer class="modal-footer"><button type="button" class="btn-text" data-modal-close>Cancelar</button>${submitLabel ? `<button type="button" class="btn btn-${variant}" data-modal-submit>${submitLabel}</button>` : ''}</footer></section>`;
    const body = overlay.querySelector('.modal-body'); if (typeof content === 'string') body.innerHTML = content; else body.append(content);
    document.body.append(overlay); document.body.classList.add('modal-open'); activeModal = overlay; window.lucide?.createIcons({ root: overlay });
    overlay.querySelectorAll('[data-modal-close]').forEach(button => button.addEventListener('click', closeModal));
    overlay.addEventListener('click', event => { if (event.target === overlay && !body.querySelector('form')) closeModal(); });
