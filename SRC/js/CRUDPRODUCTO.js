(function () {
  ...
  form.addEventListener('submit', async event => {
    ...
    if (!producto.codigodebarra) return mostrarToast('Escribe o escanea el código de barras.', 'warning', 'Falta información');
    if (!producto.nombre) return mostrarToast('Escribe el nombre del producto.', 'warning', 'Falta información');
    if (!Number.isFinite(producto.precio) || producto.precio < 0) return mostrarToast('Indica un precio válido, igual o mayor que cero.', 'warning', 'Revisa el precio');
    if (!Number.isInteger(producto.stock) || producto.stock < 0) return mostrarToast('Indica una cantidad de stock entera, igual o mayor que cero.', 'warning', 'Revisa el stock');
    if (!producto.categoria_id) return mostrarToast('Selecciona la categoría del producto.', 'warning', 'Falta información');
    ...
    catch (error) { console.error('[Products][save]', error); mostrarToast(mensajeParaUsuario(error, 'No fue posible guardar el producto. Revisa los datos e inténtalo nuevamente.'), 'danger', 'No se pudo guardar'); }
  ...
  async function cambiarStock(id, tipo) {
    ...
    const modal = SiproUI.openModal({ title: `Registrar ${action}`, subtitle: 'Actualiza el stock de forma segura', icon: action === 'entrada' ? 'arrow-down-to-line' : 'arrow-up-from-line', content: `<div><p><strong>${escapeHtml(producto.nombre)}</strong></p><p>Stock actual: ${producto.stock}</p><label for="movementQuantity">Cantidad</label><input id="movementQuantity" class="form-control" type="number" min="1" step="1" autofocus></div>`, submitLabel: `Registrar ${action}`, variant: action === 'entrada' ? 'success' : 'primary', onSubmit: async overlay => { const input = overlay.querySelector('#movementQuantity'); const cantidad = Number(input.value); if (!Number.isInteger(cantidad) || cantidad <= 0) return mostrarToast('La cantidad debe ser un entero mayor que cero.', 'warning', 'Revisa la cantidad'); const button = overlay.querySelector('[data-modal-submit]'); button.disabled = true; button.textContent = 'Registrando...'; try { const result = action === 'entrada' ? await window.electronAPI.entradaProducto(id, cantidad) : await window.electronAPI.salidaProducto(id, cantidad); if (!result) return mostrarToast('No hay suficiente stock para registrar esta salida.', 'warning', 'Stock insuficiente'); SiproUI.closeModal(); await cargarProductos(); mostrarToast(`${action === 'entrada' ? 'Entrada' : 'Salida'} registrada correctamente`, 'success'); } catch (error) { console.error(`[Products][${action}]`, error); mostrarToast(mensajeParaUsuario(error, 'No fue posible registrar el movimiento. Inténtalo nuevamente.'), 'danger', 'No se pudo registrar'); } finally { button.disabled = false; button.textContent = `Registrar ${action}`; } } });
