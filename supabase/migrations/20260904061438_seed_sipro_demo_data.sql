-- Datos ficticios y realistas para validar SIPRO como entorno de producción.
-- La carga es idempotente y solo afecta objetos con prefijo sipro_.
...
on conflict (codigodebarra) do update set
...
select p.id, p.nombre, 'ajuste', p.stock, 'Carga inicial de demostración SIPRO',
       now() - ((p.id % 14) || ' days')::interval
from public.sipro_productos p
where not exists (
  select 1 from public.sipro_movimientos_stock m
  where m.producto_id = p.id and m.nota = 'Carga inicial de demostración SIPRO'
);
